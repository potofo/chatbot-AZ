################################################################################
# Name:
#   Simple get information about collection program on vector database
# Description:
#   This is sample program for get informations about collection on vector database
# Requirement:
#   chroma
...
################################################################################
# discover detail informations about collection
if len(chroma_collections) > 0:
    print(f'chroma_collections[0]  :{chroma_collections[0]}')
    ############################################################################
    # Display the first item only if the EVALRAG collection exists
    if chroma_collections[0].name == CHROMA_SERVER_VECTORDB_COLLECTION:
        print(f'Collection name is {CHROMA_SERVER_VECTORDB_COLLECTION}')
        collection = client.get_collection(CHROMA_SERVER_VECTORDB_COLLECTION)
        ...
        # display last item
        print('num of items:{0}'.format(str(len(items['ids']))))
    ...
    print('no found collection in vector database')
